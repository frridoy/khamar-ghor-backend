...
@section('page-title', 'Define Data Schema: ' . $category->name)
...
<div style="max-width: 1100px; margin: 0 auto;">
    ...
    <div style="background: white; border: 1px solid var(--border-color); border-radius: 1rem; padding: 1.5rem; margin-bottom: 2rem; display: flex; align-items: center; justify-content: space-between; box-shadow: var(--shadow-sm);">
        <div style="display: flex; align-items: center; gap: 1.25rem;">
            ...
            <div>
                <h2 style="font-size: 1.25rem; font-weight: 800; color: var(--text-primary); margin: 0;">{{ $category->name }}</h2>
                <p style="margin: 0; font-size: 0.875rem; color: var(--text-secondary);">Add fields that will appear when vendors post products in this category.</p>
            </div>
        </div>
        <a href="{{ route('admin.categories.attributes.index', $category->id) }}" class="btn" style="background: #f1f5f9; color: var(--text-secondary);">
            <i class="fas fa-arrow-left"></i> Current Attributes
        </a>
    </div>
    ...
    <div class="card" style="padding: 2rem;">
        <form action="{{ route('admin.categories.attributes.store', $category->id) }}" method="POST">
            ...
            <div style="margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center;">
                <h3 style="font-size: 1rem; font-weight: 700; color: var(--text-primary);">Attribute List</h3>
                <span style="font-size: 0.75rem; font-weight: 700; color: var(--primary-color); text-transform: uppercase;">Build Schema</span>
            </div>
                ...
                <div class="attribute-row" style="background: #ffffff; padding: 1.25rem; border-radius: 0.75rem; margin-bottom: 1rem; border: 1px solid var(--border-color); position: relative; transition: all 0.2s; box-shadow: var(--shadow-sm);">
                    <div style="display: grid; grid-template-columns: 2fr 1.5fr 1fr 1fr 0.5fr; gap: 1.25rem;">
                        <div class="form-group" style="margin-bottom: 0;">
                            ...
                            <input type="text" name="attributes[0][name]" class="form-control" required placeholder="e.g. Color, Height, CC">
                        </div>
                        <div class="form-group" style="margin-bottom: 0;">
                            <label class="form-label" style="font-size: 0.75rem; color: var(--text-secondary);">Data Type</label>
                            <select name="attributes[0][data_type]" class="form-control" required>
                        ...
                        <div class="form-group" style="margin-bottom: 0;">
                            ...
                            <input type="text" name="attributes[0][unit]" class="form-control" placeholder="kg, ft, etc.">
                        </div>
                        <div class="form-group" style="margin-bottom: 0;">
                            <label class="form-label" style="font-size: 0.75rem; color: var(--text-secondary);">Sort Order</label>
                            <input type="number" name="attributes[0][display_order]" class="form-control" value="0">
                        </div>
                        <div style="display: flex; align-items: center; justify-content: flex-end; padding-top: 1.5rem;">
                             <label style="display: flex; flex-direction: column; align-items: center; cursor: pointer;">
                                ...
                                <input type="checkbox" name="attributes[0][is_required]" style="width: 18px; height: 18px; accent-color: var(--primary-color);">
                             </label>
                        </div>
                    </div>
                </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 1rem; border-top: 1px solid var(--border-color); padding-top: 2rem;">
                ...
                <button type="submit" class="btn btn-primary" style="padding: 0.75rem 2.5rem; font-weight: 700; box-shadow: var(--shadow-md);">
                    <i class="fas fa-check-circle" style="margin-right: 0.5rem;"></i> Save Configuration
                </button>
            </div>
        </form>
    </div>
