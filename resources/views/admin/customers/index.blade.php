...
<div class="card">
    ...
    <div style="background: #f8fafc; padding: 1.5rem; border-radius: 0.75rem; margin-bottom: 2rem; border: 1px solid var(--border-color);">
        <form action="{{ route('admin.customers.index') }}" method="GET" id="filter-form" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; align-items: flex-end;">
            <div class="form-group" style="margin-bottom: 0;">
                <label class="form-label" style="font-size: 0.75rem;">Search</label>
                <input type="text" name="search" value="{{ $search }}" class="form-control" placeholder="Name, email, phone or code..." style="padding: 0.5rem 0.75rem;">
            </div>
            ...
            <div style="display: flex; gap: 0.5rem;">
                ...
                <a href="{{ route('admin.customers.index') }}" class="btn" style="background: white; border: 1px solid #e2e8f0; color: var(--text-secondary); padding: 0.5rem 1rem;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div style="font-size: 0.75rem; color: var(--text-secondary);">Joined {{ $user->created_at->format('d M Y') }}</div>
                    </td>
                    <td>
                        <div>{{ $user->email }}</div>
                    ...
                    <td style="text-align: right;">
                        <button onclick="openModal('{{ route('admin.customers.show', $user->id) }}')" class="btn btn-sm" style="background: #f8fafc; color: var(--text-primary); border: 1px solid #e2e8f0;" title="View Details">
                            <i class="fas fa-eye"></i>
                        </button>
                        <a href="{{ route('admin.customers.edit', $user->id) }}" class="btn btn-sm" style="background: #f8fafc; color: var(--primary-color); border: 1px solid #e2e8f0;" title="Edit Customer">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form action="{{ route('admin.customers.delete', $user->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this customer?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm" style="background: #fff1f2; color: var(--danger-color); border: 1px solid #fecaca;" title="Delete Customer">
...
@section('styles')
<style>
    /* Pagination styling for Tailwind compatibility in standard CSS */
    .pagination {
        display: flex;
        list-style: none;
        gap: 0.25rem;
