...
<div class="card">
    ...
    <div style="background: #f8fafc; padding: 1.5rem; border-radius: 0.75rem; margin-bottom: 2rem; border: 1px solid var(--border-color);">
        <form action="{{ route('admin.posts.index') }}" method="GET" id="filter-form" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 1rem; align-items: flex-end;">
            <div class="form-group" style="margin-bottom: 0;">
                <label class="form-label" style="font-size: 0.75rem;">Search</label>
                <input type="text" name="search" value="{{ $search }}" class="form-control" placeholder="Title or slug..." style="padding: 0.5rem 0.75rem;">
            </div>
            ...
            <div class="form-group" style="margin-bottom: 0;">
                ...
                <input type="number" step="0.01" name="min_price" value="{{ $minPrice }}" class="form-control" placeholder="0.00" style="padding: 0.5rem 0.75rem;">
            </div>

            <div class="form-group" style="margin-bottom: 0;">
                <label class="form-label" style="font-size: 0.75rem;">Max Price (৳)</label>
                <input type="number" step="0.01" name="max_price" value="{{ $maxPrice }}" class="form-control" placeholder="999999.99" style="padding: 0.5rem 0.75rem;">
            </div>
            ...
            <div style="display: flex; gap: 0.5rem;">
                ...
                <a href="{{ route('admin.posts.index') }}" class="btn" style="background: white; border: 1px solid #e2e8f0; color: var(--text-secondary); padding: 0.5rem 1rem;">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div style="font-weight: 700; color: var(--text-primary);">৳{{ number_format($post->discount_price ?? $post->original_price, 2) }}</div>
                        @if($post->discount_price)
                            <div style="font-size: 0.75rem; color: var(--text-secondary); text-decoration: line-through;">৳{{ number_format($post->original_price, 2) }}</div>
                        @endif
                    </td>
                    <td>
                    ...
                    <td style="text-align: right;">
                        ...
                        <a href="#" class="btn btn-sm" style="background: #f8fafc; color: var(--primary-color); border: 1px solid #e2e8f0;" title="Edit Post">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form action="{{ route('admin.posts.delete', $post->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this post and all its media?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm" style="background: #fff1f2; color: var(--danger-color); border: 1px solid #fecaca;" title="Delete Post">
                ...
                <tr>
                    <td colspan="6" style="text-align: center; padding: 4rem; color: var(--text-secondary);">
                        <i class="fas fa-store-slash" style="font-size: 2.5rem; display: block; margin-bottom: 1rem; opacity: 0.2;"></i>
